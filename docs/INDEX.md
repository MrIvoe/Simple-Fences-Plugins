# Documentation Index

Use this page as the documentation landing sidebar/index.

## Start Here

- [How It Works](HOW_IT_WORKS.md)
- [Plugins Offered](PLUGINS.md)
- [Create A Plugin](CREATE_A_PLUGIN.md)
- [Release Guide](RELEASE.md)
- [Changelog](CHANGELOG.md)

## Maintenance Docs

- [Host Integration](HOST_INTEGRATION.md)
- [Theme Package Contract](THEME_PACKAGE_CONTRACT.md)
- [Plugin README Template](PLUGIN_README_TEMPLATE.md)
- [Unused File Audit](UNUSED_FILE_AUDIT.md)

## Recommended Reading Paths

### New contributor path
1. [How It Works](HOW_IT_WORKS.md)
2. [Create A Plugin](CREATE_A_PLUGIN.md)
3. [Plugin README Template](PLUGIN_README_TEMPLATE.md)
4. [Release Guide](RELEASE.md)

### Maintainer release path
1. [Changelog](CHANGELOG.md)
2. [Release Guide](RELEASE.md)
3. [Plugins Offered](PLUGINS.md)
4. [Host Integration](HOST_INTEGRATION.md)
